...
function owlAnimateFilter(){$(this).addClass('item-scale')
window.setTimeout(()=>{$(this).removeClass('item-scale')},500)}
function repositionModals(){const $dialog=$(this).find('.modal-dialog')
$(this).css('display','block')
$dialog.css('margin-top',Math.max(0,($window.height()-$dialog.height())/2))}
function initOnScrollFunctions(){const scroll=$window.scrollTop()
if(scroll>=100){$('.is-fixed').addClass('color-fill')}else{$('.is-fixed').removeClass('color-fill')}}
document.addEventListener('DOMContentLoaded',function(){const accordionHeads=document.querySelectorAll('.acod-head')
accordionHeads.forEach(function(head){head.addEventListener('click',function(){const parent=this.closest('.faq-1')
...
if(stickyHeader){const headerOffset=stickyHeader.offsetTop
...
function initCommonFunctions(){$('iframe[src*="youtube.com"], iframe[src*="vimeo.com"]').each(function(){$(this).wrap('<div class="ratio ratio-16x9"></div>')})
...
$('button.scroltop').on('click',(e)=>{e.preventDefault()
...
$('.has-child a + .submenu-toogle').on('click',function(ev){$(this).parent().siblings('.has-child').children('.sub-menu, .mega-menu').slideUp(500).parent().removeClass('nav-active')
...
function initCounterUp(){const counters=document.querySelectorAll('.counter')
if(!counters.length)return
const observerOptions={root:null,rootMargin:'0px',threshold:0.5}
const observer=new IntersectionObserver((entries,obs)=>{entries.forEach(entry=>{if(entry.isIntersecting){const el=entry.target
const finalValue=parseFloat(el.innerText.replace(/,/g,''))
const duration=5000
const start=performance.now()
const animate=(currentTime)=>{const elapsed=currentTime-start
...
requestAnimationFrame(animate)}})},observerOptions)
counters.forEach(counter=>observer.observe(counter))}
initCounterUp()}
function initOnLoadFunctions(){if($.fn.isotope){const $container=$('.masonry-outer')
$container.isotope({itemSelector:'.masonry-item',transitionDuration:'1s',originLeft:!0,stamp:'.stamp'})
$('.masonry-filter li').on('click',function(){const selector=$(this).find('a').attr('data-filter')
...
$('.btn-filter-wrap').on('click','.btn-filter',function(){const filter_data=$(this).data('filter')
...
$window.on('scroll',function(){initOnScrollFunctions()})})(jQuery)