...
;window.RevColor = {
  ...
  transparentRgba: function (val, processed) {
    if (!processed) {
      const tpe = RevColor.process(val)[1]
  ...
  rgbValues: function (values, num) {
    ...
    for (let i = 0; i < num; i++) values[i] = values[i].trim()
    return values
  },
  ...
  processGradient: function (obj) {
    const tpe = obj.type
    const begin = tpe + '-gradient('
    const middle = tpe === 'linear' ? obj.angle + 'deg, ' : 'ellipse at center, '
...
(function ($) {
  ...
  const hitWidth = 265
  const maxPoints = 20
  const centerWheel = 30
  const minGradRows = 6
  const minColorRows = 5
  const presetColumns = 6
  const deleteBuffer = 10
  const warningBuffer = 15
  const sliderHeight = 180
  const dragObj = { axis: 'x', containment: '#rev-cpicker-point-wrap' }
  ...
  function newPreset (val, core, cls, grad) {
    ...
    if (val !== 'blank') {
      ...
      let shell = '<span class="rev-cpicker-color tptip' + cls + '" data-title="' + titl + '" data-color="' + val + '">' +
  ...
  function checkPreset () {
    const presetColor = getAttribute(this, 'data-color').toLowerCase()
    const toCheck = !supressCheck ? presetColor === openingColor.toLowerCase() : false
  ...
  function writePresets (container, colors) {
    ...
    const core = container.search('core') !== -1
    const cls = core ? '' : ' rev-picker-color-custom'
    let len = colors.length
    const minRows = container.search('colors') !== -1 ? minColorRows : minGradRows
    const rows = Math.max(Math.ceil(len / presetColumns), minRows)
  ...
  function onChange (gradient, color, trans) {
    ...
    if (!gradient) {
      const hex = color || colorHex.val()
      const opacity = typeof trans !== 'undefined' ? trans : colorOpacity.val()
  ...
  function setValue (val, fromPreset) {
    const obj = RevColor.process(val)
    const type = obj[1]
    const clr = obj[0]
    ...
    if (type !== 'gradient') {
      switch (type) {
        ...
        case 'rgba':
          ...
          var opacity = parseInt(values[1] * 100, 10)
          val = values[0]
  ...
  function clonePoint (align, pos) {
    const sibling = getSibling(align, pos)
    const clr = sibling.color
    const bg = getBgVal(clr, align, true)
    const rgb = getRgbVal(clr, true)

    const pnt = newPoint(align, pos, rgb, bg)
  ...
  function activatePoint (pointX) {
    ...
    let color = curPoint.attr('data-color')
    const bottom = curPoint.hasClass('rev-cpicker-point-bottom')
    ...
    if (bottom) {
      ...
      const opacity = RevColor.getOpacity(color)
  ...
  function checkGroup () {
    const $this = $(this)
    const len = $this.children('.rev-cpicker-color').not('.blank').length
  ...
  function checkRows () {
    const $this = $(this)
    const minRows = this.id.search('colors') !== -1 ? minColorRows : minGradRows
    const colors = $this.children('.rev-cpicker-color')
    let len = colors.length
    const rows = Math.ceil(len / presetColumns)
    const minItems = minRows * presetColumns
    let rowRemoved

    len += 1
    for (let i = 0; i < rows; i++) {
      const start = i * presetColumns
      const range = colors.slice(start, parseInt(start + presetColumns, 10) - 1)
  ...
  function buildGradientOutput (el, revrse, save) {
    ...
    const gradient = RevColor.processGradient(gradientPreview)
  ...
  function adjustAlpha (point, begin, end) {
    let val
    const beginAlpha = begin.alpha
    const endAlpha = end.alpha
  ...
  function getPoints (i) {
    const rgba = RevColor.rgbValues(getAttribute(this, 'data-color'), 4)
    const align = this.className.search('bottom') !== -1 ? 'bottom' : 'top'
    const alpha = rgba[3].replace(/\.?0*$/, '') || 0
    let pos = parseInt(this.style.left, 10)
  ...
  function onAngleChange (e, dir, wheel) {
    const isWheel = typeof wheel !== 'undefined'
    let val = isWheel ? wheel : parseInt(angle.val(), 10)
    let changed
    let value
    ...
    if (e && e.type === 'keyup') {
      ...
      const oValue = parseInt(angle.data('orig-value'), 10)
  ...
  function onDrag (e, ui) {
    const pos = parseInt((Math.round(ui.position.left) / (hitWidth - 2)).toFixed(2) * 100, 10)
  ...
  function wheelMove (e) {
    const offset = angleWheel.offset()
    const posX = e.pageX - offset.left
    const posY = e.pageY - offset.top

    if (isNaN(posX) || isNaN(posY)) return

    let val = Math.atan2(posY - centerWheel, posX - centerWheel) * (180 / Math.PI) + 90
    if (val < 0) val += 360
  ...
  function init () {
    ...
    $('.rev-cpicker-main-btn').on('click.revcpicker', function () {
      ...
      if (state) {
        const isTrans = state === 'transparent'
        const val = !isTrans ? state : ''
    ...
    $('#rev-cpicker-check').on('click.revcipicker', function () {
      ...
      if (cPicker.hasClass('color-view')) {
        ...
        const opacity = colorOpacity.val()
        ...
        const grad = buildGradientOutput(false, false, true)
        const obj = $.extend({}, grad[0])
        const clr = grad[1]
    ...
    cPicker.on('click.revcpicker', function (e) {
      if (cPicker.hasClass('open')) {
        ...
        const $this = $(targt)
        const ids = targt.id
        let isTarget = targt.className.search('rev-cpicker-point') !== -1 ||
    ...
    gradIris.wpColorPicker({
      ...
      change: function (e, ui) {
        ...
        const rgba = RevColor.processRgba(val, 100)
        const parsed = RevColor.rgbValues(rgba, 4)
        const color = gradientPreview.colors[pointIndex]
    ...
    opacitySlider.slider({
      ...
      slide: function (e, ui) {
        if (!supressOpacity) {
          const opacity = parseInt((ui.value / sliderHeight).toFixed(2) * 100, 10)
          let val
    ...
    $('.rev-cpicker-point-location').on('keyup.revcpicker focusout.revcpicker', function (e) {
      ...
      const align = curPoint.hasClass('rev-cpicker-point-bottom') ? 'bottom' : 'top'
      const locations = align === 'bottom' ? botPoints : topPoints
      const input = align === 'bottom' ? colorLoc : opacityLoc
      let point = input.val().replace('%', '') || '0'
      const evt = e.type
      let dir
    ...
    $('body').on('click.revcpicker', '.rev-cpicker-point', function () {
      ...
      const start = curPoint.data('mousestart')
      const posY = e.pageY
      ...
      const $this = $(this)
      let val = $this.data('gradient') || $this.val() || 'transparent'
      ...
      const ids = selectedColor.parent()[0].id
      const tpe = ids.search('core') !== -1 ? 'core' : 'custom'
      const view = ids.search('colors') !== -1 ? 'colors' : 'gradients'
      const btn = $('#rev-cpicker-' + view + '-' + tpe + '-btn')

      if (btn.hasClass('active')) btn.find('.rev-cpicker-arrow-up').click()
      if (cPicker.hasClass('color-view')) {
        let val = selectedColor.attr('data-color')
      ...
      if (window.confirm(document.getElementById('rev-cpicker-remove-delete').innerHTML)) {
        ...
        const $this = $(this)
        const colr = $this.parent()
        let titl = colr.attr('data-title') || ''
        ...
        const group = $this.closest('.rev-cpicker-presets-group')
        const ids = group[0].id
        const tpe = ids.search('colors') !== -1 ? 'colors' : 'gradients'

        doc.off('revcpicker_onajax_delete.revcpicker').on('revcpicker_onajax_delete.revcpicker', function (evt, err) {
          if (err) console.log(err)

          const group = $this.closest('.rev-cpicker-presets-group')
          const scroller = group.find('.ps-scrollbar-x-rail')
          const btn = $('#' + ids + '-btn')
    ...
    $('.rev-cpicker-save-preset-btn').on('click.revcpicker', function () {
      ...
      let duplicateTitle
      const $this = $(this)
      const input = $this.closest('.rev-cpicker-presets-save-as').find('.rev-cpicker-preset-save')
      let titl = input.val()
      ...
      $('#rev-cpicker-' + presetGroup + '-custom').find('.rev-cpicker-color').not('.blank').each(function () {
        const atr = $.trim(getAttribute(this, 'data-title').replace(/\W+/g, '_')).replace(/^\_|\_$/g, '').toLowerCase()
        if (atr === titl) {
          alert($this.attr('data-message'))
          duplicateTitle = true
          return false
      ...
      doc.off('revcpicker_onajax_save.revcpicker').on('revcpicker_onajax_save.revcpicker', function (evt, err) {
        ...
        const pre = $(newPreset(newColorValue, false, ' rev-picker-color-custom', grad))
        const group = $('#rev-cpicker-' + presetGroup + '-custom')
        const box = group.find('.rev-cpicker-color.blank')
        const btn = $('#' + group[0].id + '-btn')
        ...
        const len = checkGroup.call(group[0])
    ...
    $('.rev-cpicker-preset-title').on('click.revcpicker', function () {
      ...
      const arrow = $this.hasClass('active') ? 'down' : 'up'
    ...
    $('.rev-cpicker-hex').on('focusout.revcpicker', function () {
      ...
      if (this.id === 'rev-cpicker-color-hex') {
        ...
        if (!RevColor.isColor.test(val)) {
          $this = $(this)
          oVal = $this.data('orig-value')
          val = oVal || RevColor.defaultValue
        }
    ...
    $('.rev-cpicker-point-delete').on('click.revcpicker', function () {
      ...
      const align = curPoint.hasClass('rev-cpicker-point-bottom') ? 'bottom' : 'top'
      const len = cPicker.find('.rev-cpicker-point-' + align).length
    ...
    $('.rev-cpicker-opacity-input').on('keyup.revcpicker focusout.revcpicker', function (e) {
      const isColor = this.id.search('grad') === -1
      const $this = isColor ? colorOpacity : gradOpacity
      let opacity = $this.val().replace('%', '')
      const evt = e.type
      let clr
      ...
      if (isColor) {
        const opaque = parseInt(opacity, 10)
        const val = opaque !== 0 ? false : 'transparent'

        onChange(false, val, opacity)
        updateSlider(opacity)
      } else {
        const parsed = RevColor.rgbValues(curPoint.attr('data-color'), 3)
        const color = gradientPreview.colors[pointIndex]
    ...
    $('.rev-cpicker-builder-hit').on('click.revcpicker', function (e) {
      ...
      let hit = parseInt(((e.pageX - hitTop.offset().left) / hitWidth).toFixed(2) * 100, 10)
      const align = this.id.search('bottom') !== -1 ? 'bottom' : 'top'
      const locations = align === 'bottom' ? botPoints : topPoints
      const dir = hit < 50 ? 1 : -1
    ...
    reverse.on('click.revcpicker', function () {
      const rev = !reverse.hasClass('checked')
  ...
  const ColorPicker = {
    ...
    refresh: function () {
      ...
      if ($this.hasClass('rev-cpicker-component')) {
        const settings = $this.data('revcp') || {}
        let val = $this.val() || settings.defaultValue || RevColor.defaultValue
        let colorValue = RevColor.process(val)
  ...
  $.fn.tpColorPicker = function (settings) {
    ...
    return this.each(function () {
      ...
      const wrap = $('<span class="rev-colorpicker"></span>').data('revcolorinput', $this)
      const box = $('<span class="rev-colorbox" />')
      const btn = $('<span class="rev-colorbtn" />')
      let cls = $this.attr('data-wrap-classes')
      let wrapper = $this.attr('data-wrapper')
      let ids = $this.attr('data-wrap-id')
      let txt = $this.attr('data-title')
      let skin = $this.attr('data-skin')
      let val = $this.val() || ''
      const falsey = isFalsey(val)
      let colorValue
      let defValue
      ...
      const initCallback = settings ? settings.init || onInit : false
  ...
  $(function () {
    $('body').on('click.revcpicker', '.rev-colorpicker', function () {
      ...
      let widgetId = currentInput.attr('data-widget-id')
      let html = currentInput.attr('data-appended-html')
      let editing = currentInput.attr('data-editing')
      let data = currentInput.attr('data-colors')
      let mode = currentInput.attr('data-mode')
      const settings = currentInput.data('revcp')
      let lang = currentInput.attr('data-lang')
      let settingsGradients
      let customGradients
      let settingsColors
      let dataGradients
      let customColors
      ...
      if (settings) {
        const colorSets = settings.colors
        if (colorSets) {
          if (colorSets.core) {
