...
;(function (jQuery, undefined) {
  ...
  jQuery.fn.extend({
    ...
    getRSVersion: function (silent) {
      if (silent === true) {
        ...
        const v = jQuery('body').data('tp_rs_version')
        let t = ''
        t += ('---------------------------------------------------------') + '\n'
  ...
  jQuery.extend(true, _R, {
    ...
    compare_version: function (extension) {
      let v = jQuery('body').data('tp_rs_version')
    ...
    currentSlideIndex: function (opt) {
      let	ai = opt.c.find('.active-revslide').index()
    ...
    isIE: function (version, comparison) {
	    const $div = jQuery('<div style="display:none;"/>').appendTo(jQuery('body'))
	...
	*/
    isSafari11: function () {
      const browser = jQuery.trim(_R.get_browser().toLowerCase())
    ...
    setSize: function (opt) {
      ...
      let __mh = 0
      const _actli = opt.nextSlide === -1 || opt.nextSlide === undefined ? 0 : opt.nextSlide
      opt.paddings = opt.paddings === undefined ? { top: (parseInt(opt.c.parent().css('paddingTop'), 0) || 0), bottom: (parseInt(opt.c.parent().css('paddingBottom'), 0) || 0) } : opt.paddings

      if (opt.rowzones && opt.rowzones.length > 0) {
        for (let a = 0; a < opt.rowzones[_actli].length; a++) {
      ...
      if (opt.sliderLayout == 'fullscreen' || opt.infullscreenmode) {
        ...
        const cow = opt.c.parent().width()
        let coh = jQuery(window).height()
        ...
        if (opt.fullScreenOffsetContainer != undefined) {
          ...
          try {
            if (opt.fullScreenOffset.split('%').length > 1 && opt.fullScreenOffset != undefined && opt.fullScreenOffset.length > 0) { coh = coh - (jQuery(window).height() * parseInt(opt.fullScreenOffset, 0) / 100) } else
      ...
      const si = {	height: (cpt + cpb + ofh + opt.height + opt.paddings.top + opt.paddings.bottom) }
  ...
  const checkIDS = function (opt, item) {
    opt.anyid = opt.anyid === undefined ? [] : opt.anyid
    const ind = jQuery.inArray(item.attr('id'), opt.anyid)
    if (ind != -1) {
      const newid = item.attr('id') + '_' + Math.round(Math.random() * 9999)
      item.attr('id', newid)
    }
  ...
  var getScriptLocation = function (a) {
    const srcexp = new RegExp('themepunch.revolution.min.js', 'gi')
    let ret = ''
  ...
  /// ///////////////////////////////////////
  const setCurWinRange = function (opt, vis) {
    const curlevel = 0
    let curwidth = 9999
    let lastmaxlevel = 0
    let lastmaxid = 0
  ...
  var initSlider = function (container, opt) {
    ...
    if (container.data('aimg') != undefined) {
      if ((container.data('aie8') == 'enabled' && _R.isIE(8)) || (container.data('amobile') == 'enabled' && _ISM)) { container.html('<img class="tp-slider-alternative-image" src="' + container.data('aimg') + '">') }
    }
  ...
  var runSlider = function (container, opt) {
    ...
    jQuery.each(opt.allli, function (index, li) {
      ...
      if (li.data('link') != undefined) {
        ...
        const jts = linktoslide === 'scroll_under'
        ...
        const frame = ' data-frames=' + "'" + '[{"delay":0,"speed":100,"frame":"0","from":"opacity:0;","to":"o:1;","ease":"Power3.easeInOut"},{"delay":"wait","speed":300,"frame":"999","to":"opacity:0;","ease":"Power3.easeInOut"}]' + "'"
    ...
    if (opt.sliderLayout != 'auto' && container.closest('.forcefullwidth_wrapper_tp_banner').length == 0) {
      if (opt.sliderLayout !== 'fullscreen' || opt.fullScreenAutoWidth != 'on') {
        ...
        const cid = container.attr('id') + '_forcefullwidth'
    ...
    if (!container.hasClass('revslider-initialised')) {
      ...
      const versionIncrement = parseFloat(version[2] || '0')
      ...
      container.find('.tp-caption, .rs-background-video-layer').each(function (i) {
        const _nc = jQuery(this)
        const _ = _nc.data()
        const an = _.autoplayonlyfirsttime
        const ap = _.autoplay
        const htmlvideo = _.videomp4 !== undefined || _.videowebm !== undefined || _.videoogv !== undefined
        const al = _nc.hasClass('tp-audiolayer')
        const loop = _.videoloop
        let addtofadeout = true
        let addToStaticFadeout = false
        ...
        if (_nc.hasClass('tp-caption')) {
          ...
          const _ndata = _nc.data()
          let specec = ''
          const nctype = _ndata.type
          const _pos = nctype === 'row' || nctype === 'column' ? 'relative' : 'absolute'
          let preclas = ''
          ...
          if (addtofadeout && opt.scrolleffect.on) {
            if ((opt.scrolleffect.on_parallax_layers === 'on' && _.isparallaxlayer) || (opt.scrolleffect.on_layers === 'on' && !_.isparallaxlayer)) { opt.scrolleffect.layers.push(_nc.parent()) }
          }
      ...
      if (_v != undefined && _v.enable) {
        if (!jQuery.isNumeric(_v.visible_area)) {
          if (_v.visible_area.indexOf('%') !== -1) { _v.visible_area = parseInt(_v.visible_area) / 100 }
        }
      ...
      jQuery(window).on(resizid, function () {
        ...
        if (opt.sliderLayout == 'fullscreen') {
          const jwh = jQuery(window).height()
...
**************************************/
  var cArray = function (b, l) {
    ...
    if (b.length < l) {
      var t = b[b.length - 1]
      for (let i = 0; i < (l - b.length) + 2; i++) { b.push(t) }
    }
    return b
  }
  ...
  var checkHoverDependencies = function (_nc, opt) {
    const _ = _nc.data()
    const senter = !!(_.start === 'sliderenter' || (_.frames !== undefined && _.frames[0] != undefined && _.frames[0].delay === 'sliderenter'))
        ...
        opt.c.on('tp-mouseenter', function () {
          if (opt.layersonhover) {
            jQuery.each(opt.layersonhover, function (i, tnc) {
              const cli = tnc.data('closestli') || tnc.closest('.tp-revslider-slidesli')
              const stl = tnc.data('staticli') || tnc.closest('.tp-static-layers')
              ...
              if ((cli.length > 0 && (cli.hasClass('active-revslide')) || cli.hasClass('processing-revslide')) || (stl.length > 0)) {
                ...
                if (_R.playAnimationFrame) { _R.playAnimationFrame({ caption: tnc, opt, frame: 'frame_0', triggerdirection: 'in', triggerframein: 'frame_0', triggerframeout: 'frame_999' }) }
        ...
        opt.c.on('tp-mouseleft', function () {
          if (opt.layersonhover) {
            jQuery.each(opt.layersonhover, function (i, tnc) {
              ...
              if (_R.playAnimationFrame) _R.playAnimationFrame({ caption: tnc, opt, frame: 'frame_999', triggerdirection: 'out', triggerframein: 'frame_0', triggerframeout: 'frame_999' })
            })
          }
        })
  ...
  var containerResized = function (c, opt) {
    ...
    if (!_R.resizeThumbsTabs || _R.resizeThumbsTabs(opt) === true) {
        ...
        opt.pcartimer = setTimeout(function () {
          ...
          if (opt.sliderType == 'carousel' && opt.carousel.showLayersAllTime === 'on') {
            jQuery.each(opt.li, function (i) {
              _R.animateTheCaptions({ slide: jQuery(opt.li[i]), opt, recall: true })
            })
          }
        }, 100)
      ...
      if (_R.animateTheCaptions && c.find('.active-revslide').length > 0) { _R.animateTheCaptions({ slide: c.find('.active-revslide'), opt, recall: true }) }
      ...
      if (_R.animateTheCaptions && c.find('.processing-revslide').length > 0) _R.animateTheCaptions({ slide: c.find('.processing-revslide'), opt, recall: true })
      if (_R.manageNavigation) _R.manageNavigation(opt)
    }
  ...
  var prepareSlides = function (container, opt) {
    ...
    opt.allli.find('>img').each(function (j) {
      ...
      const cli = img.closest('li')
      const bgvid = cli.find('.rs-background-video-layer')
      ...
      let mediafilter = cli.data('mediafilter')
      ...
      const bgcolor = img.data('bgcolor')
      let bgstyle = ''
      ...
      const newimg = jQuery('<div class="tp-bgimg defaultimg ' + mediafilter + '" data-bgcolor="' + bgcolor + '" style=' + bgstyle + '></div>')
  ...
  const cutParams = function (a) {
    let b = a
    if (a != undefined && a.length > 0) { b = a.split('?')[0] }
    return b
  }

  const relativeRedir = function (redir) {
    return location.pathname.replace(/(.*)\/[^/]*/, '$1/' + redir)
  }

  const abstorel = function (base, relative) {
  ...
  const imgLoaded = function (img, opt, progress) {
    opt.syncload--
    if (opt.loadqueue) {
      jQuery.each(opt.loadqueue, function (index, queue) {
        ...
        const fullsrc_b = fullsrc.substring(0, fullsrc.length - 1) + '/' + mqsrc
        const origin_b = origin + '/' + mqsrc
  ...
  const addToLoadQueue = function (src, opt, prio, type, staticlayer) {
  ...
  var waitForCurrentImages = function (nextli, opt, callback) {
    ...
    nextli.find('img,.defaultimg, .tp-svg-layer').each(function () {
      ...
      if (loadobj && loadobj.progress && loadobj.progress.match(/inprogress|inload|prepared/g)) {
        if (!loadobj.error && jQuery.now() - element.data('start-to-load') < 5000) { waitforload = true } else {
  ...
  var swapSlide = function (container) {
    ...
    let nextli = container.find('.next-revslide')
    ...
    if (opt.spinner === 'off') {
      ...
      else { opt.loadertimer = setTimeout(function () { if (opt.loader !== undefined) opt.loader.css({ display: 'block' }) }, 50) }
    }
  ...
  var swapSlideProgress = function (defimg, container) {
    ...
    if (opt.sliderType == 'carousel') {
      ...
      if (_R.animateTheCaptions) { _R.animateTheCaptions({ slide: nextli, opt, preset: true }) }
      ...
      if (opt.sliderType === 'carousel' && opt.carousel.showLayersAllTime === 'on') {
        ...
        jQuery.each(opt.li, function (i) {
          if (!opt.carousel.allLayersStarted) {
            if (opt.li[i] === nextli) { _R.animateTheCaptions({ slide: jQuery(opt.li[i]), maintimeline: mtl, opt, startslideanimat: 0 }) } else { _R.animateTheCaptions({ slide: jQuery(opt.li[i]), opt, startslideanimat: 0 }) }
          } else { _R.animateTheCaptions({ slide: jQuery(opt.li[i]), opt, recall: true }) }
        })
        opt.carousel.allLayersStarted = true
      } else {
        _R.animateTheCaptions({ slide: nextli, opt, maintimeline: mtl, startslideanimat: 0 })
      }
    } else {
      if (mtl != undefined) {
        setTimeout(function () {
  ...
  var letItFree = function (container, nextsh, actsh, nextli, actli, mtl) {
    ...
    if (opt.startWithSlide !== undefined && opt.startWithSlide !== 'done' && opt.sliderType === 'carousel') {
      let callslideindex = opt.startWithSlide
      for (let lis = 0; lis <= opt.li.length - 1; lis++) {
        const oindex = jQuery(opt.li[lis]).data('originalindex')
    ...
    let _actli = opt.nextSlide === -1 || opt.nextSlide === undefined ? 0 : opt.nextSlide
  ...
  /// ////////////////////////
  const removeAllListeners = function (container, opt) {
  ...
  var countDown = function (container, opt) {
    ...
    container.on('stoptimer', function () {
      const bt = jQuery(this).find('.tp-bannertimer')
      bt[0].tween.pause()
  ...
  const restartOnFocus = function () {
    jQuery('.rev_redraw_on_blurfocus').each(function () {
      const opt = jQuery(this)[0].opt
